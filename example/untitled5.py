...
from CM import CouplingMatrix
class TransmissionZerosTable(QTableWidget):
    rowInitialized = pyqtSignal(list)
    rowsSwapped = pyqtSignal(list)

    def __init__(self, bw, f0):
        super().__init__(1, 3)
    ...
    def update_normalized_value(self, row, frequency, enable=True):
        normalized_value = CouplingMatrix.bandpass2lowpass(self.bw, self.f0, frequency)
    ...
    def emit_row_initialized(self, row):
        if self.is_row_initialized(row):
            self.rowInitialized.emit(self.get_row_data(row))
...
if __name__ == '__main__':
    ...
    mainWin = TransmissionZerosTable(200, 3400)
    mainWin.show()
