...
class WidgetData:
    ...
    def __init__(self, mainClass, parent):  # parent: SARibbonMainWindow
        self.obj: SAFramelessHelper = mainClass
...
class SAFramelessHelper(QObject):
    """设置主类窗体效果"""
    def __init__(self, parent):
        ...
        self.m_bRubberBandOnMove = True
        self.m_bRubberBandOnResize = True
        # self.m_widgetDataHash: Dict[QWidget, WidgetData] = dict() # c++此处使用了字典，但感觉没必要
        self.data: WidgetData = None    # 直接使用数据实例代替字典

        if parent:
            w: QWidget = parent
            w.setWindowFlags(w.windowFlags() | Qt.FramelessWindowHint)  # 设置无边框
            self.setWidgetMovable(True)         # 设置窗体可移动
            self.setWidgetResizable(True)       # 设置窗体可缩放
            self.setRubberBandOnMove(False)     # 设置橡皮筋效果-可移动
            self.setRubberBandOnResize(True)    # 设置橡皮筋效果-可缩放
    ...
    def activateOn(self, parent: QWidget):
        ...
        if not self.data:
            self.data = WidgetData(self, parent)
            parent.installEventFilter(self)
    ...
    def setRubberBandOnMove(self, movable: bool):
        ...
        if self.data:
            self.data.updateRubberBandStatus()
...
if __name__ == '__main__':
    ...
    framelessHelper = SAFramelessHelper(mainWindow)
